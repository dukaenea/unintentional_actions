...
class VTN(nn.Module):
    ...
    def forward(self, x, position_ids, speed_labels=None, pure_nr_frames=None, return_features=False,
                classifier_only=False, backbone_only=False, backbone_feats_only=False, high_order_temporal_features=False):
        if classifier_only:
            x = self.mlp_head(x)
            return x
...
def create_model(num_classes=None, pretrained=False):
    ...
    if opt.gpu_parallel:
        ...
        if pretrained:
            ...
            # model.module.expand_memory(256)
            # if opt.use_memory:
...
class MILRegLoss(nn.Module):
    ...
    def mil_objective(self, y_pred, batch_size):
        labmdas = 8e-5

        normal_vid_sc = y_pred[batch_size*10:].squeeze()
        anomal_vid_sc = y_pred[:batch_size*10].squeeze()
        ...
        # anomal_segs_max_scores = torch.topk(anomal_vid_sc, 3, dim=1)[0].mean(1)
