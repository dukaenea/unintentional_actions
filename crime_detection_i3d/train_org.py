...
class RTFM_loss(torch.nn.Module):
    ...
    def forward(self, score_normal, score_abnormal, nlabel, alabel, feat_n, feat_a):
        ...
        hinge_loss = 1 - score_abnormal + score_normal
        loss_cls = torch.max(torch.zeros_like(hinge_loss), hinge_loss).mean()
...
def train(nloader, aloader, model, batch_size, optimizer, device):
    with torch.set_grad_enabled(True):
        model.train()
        mil_loss = MILRegLoss(model)
        ninput, nlabel = next(nloader)
        ainput, alabel = next(aloader)

        input = torch.cat((ninput, ainput), 0).to(device)
        # input = input[:, random.randint(0, 9)].unsqueeze(1)
        ...
        scores = model(input)
        cost = mil_loss(scores, batch_size)
        cost = cost
