...
if __name__ == "__main__":
    ...
    opt.save_model = 1
    opt.pretrained = True
    opt.log_name = "lr:%f~ep:%d~bs:%d~ptr:%s_cntr_loss" % (
        opt.lr,
        opt.epochs,
        opt.batch_size,
        str(opt.pretrained),
    ...
    opt.sfx = str(
        "%s.unint_act.time%s_cntr_loss"
        % (opt.dataset, datetime.now().strftime("%Y%m%d-%H%M%S"))
    )
    ...
    train_loader = DataLoader(train_set, batch_size=256, num_workers=32, shuffle=True)
    val_loader = DataLoader(val_set, batch_size=256, num_workers=32, shuffle=False)
