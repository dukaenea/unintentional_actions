...
from rep_learning.feat_transformer import (
    merge_along_time,
    pad_time_mirror,
    transform_tensor_time,
    transform_tensor_space_time,
    center_crop,
    fast_forward,
)
...
class KineticsDataset(Dataset):
    ...
    def _preprocess_video(self, tensor):
        ...
        if self.training:
            ...
            snips = np.linspace(
                0, tensor.shape[0], num=self.test_n_splits, endpoint=False, dtype=int
            )
            tensor = tensor[snips]
    ...
    def get_video_path(self, idx):
        ...
        if self.data_level == "frames":
            video_path = ops.join(
                self.video_path, self.csv["label"][idx].replace("_", " "), video_name
            )
        else:
    ...
    def __getitem__(self, idx):
        ...
        video = (
            self._get_video(video_path)
            if self.data_level == "frames"
            else self._get_video_feats(video_path)
        )
        if video is None:
            print(video_path)
    ...
    def _foba_video(self, video, eff_skip, min_seq_len):
        ...
        sub_len = min(
            vid_len, min_seq_len if eff_skip == 1 else (min_seq_len - 1) * eff_skip
        )
        ...
        try:
            inds_foba = torch.gather(
                input=crop_idx, dim=0, index=inds_foba.type(torch.int64)
            )
        except Exception:
            print("")
        if len(video.shape) == 4:
    ...
    def trn_rec_coll_fn(self, batch):
        max_batch_len = max(
            [s["features"].shape[0] * s["features"].shape[2] for s in batch]
        )
    ...
    def feat_load_act_rec_collate_fn(self, batch):
        ...
        for i in range(len(batch)):
            ...
            if len(vid_feats.shape) == 2:
                ...
                vid_feats = center_crop(vid_feats, [min_spat_size, min_spat_size])[
                    "tensor"
                ]
    ...
    def feat_load_collate_fn(self, batch):
        ...
        for i in range(len(batch)):
            ...
            if (
                vid_feats.shape[2] == 1 and vid_feats.shape[3] == 1
            ):  # case when only having temp component
