...
class SimpleOopsDataset(Dataset):
    ...
    def _compute_clips(self):
        ...
        if ops.isfile(clips_meta_path):
            ...
            for idx in tqdm(range(len(self.csv))):
                video_path = ops.join(
                    self.base_video_path, self.csv.iloc[idx]["filename"]
                )
                video = self._get_features_video(video_path)
                if self.loc_class:
                    vid_clips, labs, ctb = self._get_clip_idx(
                        video, self.csv.iloc[idx]["t"], video_path
                    )
                else:
                    vid_clips, labs = self._get_clip_idx(
                        video, self.csv.iloc[idx]["t"], video_path
                    )
    ...
    def _get_clip_idx(self, video, transition_time, video_path):
        ...
        (
            ...
            video_tunit,
            vid_timestamps,
        ) = self._get_transition_frame(video_path, transition_time)
        ...
        p2 = (
            p1 + self.fps
        )  # + round(float(video_fps)) if self.loc_class else self.fpc # get 1 sec clips when doing localization by classification
    ...
    def _balance_video(self, indexes, labels):
        ...
        for i in labs:
            if i != lbl_mode and counts[i] > 0:
                n_to_add = round(
                    counts[i] * ((counts[lbl_mode] * ratios[i] / counts[i]) - 1)
                )
    ...
    def pad_videos_collate_clip(self, batch):
        ...
        for data in batch:
            ...
            for idx, clip in enumerate(video):
                ...
                if self.mode == "train":
                    tmp_pre_un.append(
                        {"features": clip, "label": label, "pure_nr_frames": self.fps}
                    )
                else:
                    new_batch.append(
                        {"features": clip, "label": label, "pure_nr_frames": self.fps}
    ...
    def _get_raw_video(self, video_path):
        ...
        out, _ = cmd.output("pipe:", format="rawvideo", pix_fmt="rgb24").run(
            capture_stdout=True, quiet=True, capture_stderr=True
        )
        try:
    ...
    def _get_features_video(self, video_path, clip=None):
        ...
        if self.loc_class and clip is not None:
            ...
            with av.open(
                self.base_video_path_frames + "/" + video_name, metadata_errors="ignore"
            ) as container:
                video_fps = container.streams.video[0].average_rate
                num_frames = video.shape[0] * (float(self.fps) / video_fps)
                resampling_idx = self._resample_video_idx(
                    num_frames, video_fps, self.fps
                )
                video = video[resampling_idx]
                if self.mode == "train":
    ...
    def __getitem__(self, idx):
        ...
        try:
            if self.feature_level == "features":
                ...
                video_path = ops.join(
                    self.base_video_path, self.csv.iloc[video_idx]["filename"]
                )
                if video_path[-4:] != ".mp4" and "unlabeled" in self.csv_path:
                    video_path += ".mp4"
                    output["features"] = (
                        self._get_raw_video(video_path)
                        if self.feature_level == "frames"
                        else self._get_features_video(video_path)
                    )
                ...
                if self.loc_class:
                    ...
                    output["pure_nr_frames"] = torch.tensor(
                        [output["features"].shape[1]] * (output["features"].shape[0])
                    )
                    output["clip_time_boarders"] = torch.tensor(
                        self.clip_time_borders[video_idx]
                    )
                else:
