...
from crime_detection.test import test
...
def train(**kwargs):
    ...
    if opt.lr_scheduler == 'aiayn':  # Attention is All You Need
        aiayn_scheduler = AIAYNScheduler(opt.hidden_dim,
                                         0.1 * (len(train_loader) * opt.epochs))  # 10% of the steps for the warmup
    test(model=model,
         loss=loss,
    ...
    for epoch in range(s_epoch, opt.epochs):
        ...
        for idx, data in iterator:
            pure_nr_frames = data['pure_nr_frames'].flatten()
            labels = data['label']
            keys = list(data.keys())
            ...
            position_ids = torch.tensor(list(range(0, videos.shape[2]))) \
                .expand(1, 1, videos.shape[2]) \
                .repeat(videos.shape[0], videos.shape[1], 1)
            ...
            if opt.lr_scheduler == 'aiayn':
                aiayn_scheduler.step(optimizer, (epoch * len(train_loader)) + idx)
...
def couple_batch_to_clip_batch(couple_batch, samples_shape):
    ...
    couple_batch = couple_batch.reshape(B * CO,  CL, F, C)
    return couple_batch
