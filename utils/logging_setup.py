...
def setup_logger_path():
    ...
    # close previously opened logging files if exist:
    ...
    args_iter = list(vars(opt))
    for var in sorted(args_iter):
        logger.debug('%s %s' % (var, getattr(opt, var)))

    if opt.viz:
        try:
            viz.env = visdom.Visdom(server='http://helgoland', port=8125, env=opt.viz_env) #, log_to_filename=opt.viz_path)
            assert viz.env.check_connection(), \
