...
csv_paths = ['/BS/unintentional_actions/work/data/oops/splits/train_all_org.csv']
# csv_paths = ['/BS/unintentional_actions/work/data/oops/splits/val_all_org.csv']

output_paths = ['/BS/unintentional_actions/nobackup/oops/oops_dataset/oops_video/train_downsize/']
...
def get_video_info(video_path):
    ...
    return int(video_stream['height']), int(video_stream['width']), int(int(video_stream['r_frame_rate'].split('/')[0])/100)
        ...
        for index, row in tqdm(csv.iterrows(), total=csv.shape[0]):
            ...
            try:
                ...
                ffmpeg.input(vid_path)\
                    .filter('scale', new_height, new_width)\
