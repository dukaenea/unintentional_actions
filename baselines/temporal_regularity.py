...
def comp_extrema_and_persistance(reg_scores, threshold, window):
    ...
    anom_segments = merge_anomalous_segments(
        [t[0] for t in filtered_ext_per], reg_scores.shape[0], window
    )
    ...
    for a_seg in anom_segments:
        ...
        for t_seg in gt_segments:
            ...
            # intersection on the left of a_seg and right of t_seg:
                ...
                if (
                    coverage >= (a_seg[1] - a_seg[0]) // 2
                    and coverage >= (t_seg[1] - t_seg[0]) // 2
                ):
                    tp += 1
                else:
                    fp += 1
...
if __name__ == "__main__":
    ...
    opt.num_workers = 16
    opt.sfx = str(
        "%s.conv_vae.num_in_frames%d"
        % (opt.temp_learning_dataset_name, opt.num_in_channels)
    )
    opt.save_model = 1
    opt.test_val = True
    opt.epochs = 3000
    ...
    if opt.temp_learning_dataset_name == "ped":
        train_set = PedestrianDataset(
            "Train", gray_scale=True, in_channels=opt.num_in_channels
        )
        test_set = PedestrianDataset(
            "Test", gray_scale=True, in_channels=opt.num_in_channels
        )
    elif opt.temp_learning_dataset_name == "avenue":
        train_set = AvenueDataset(
            "train",
    ...
    model, optimizer, loss = create_model(
        opt.num_in_channels, opt.lr, opt.optim, opt.weight_decay, pretrained=False
    )
