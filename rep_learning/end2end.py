...
if __name__ == "__main__":
    ...
    train_set = OopsTtibua(
        "train", spat_scale=True, size=224, spat_crop=True, load_frames=True
    )
    val_set = OopsTtibua(
        "val", spat_scale=True, size=224, spat_crop=True, load_frames=True
    )
    ...
    model, optimizer, loss = create_model(
        num_classes=(None if opt.pretrained else 8), pretrained=opt.pretrained
    )
