...
def vis_tsne_base():
    base_split_path = "/BS/feat_augm/work/metadata/kinetics/data_splits"
    base_feats_paths = "/BS/feat_augm/work/metadata/kinetics/c3d_cnn_nn"
    labels_path = "/BS/feat_augm/work/metadata/kinetics/data_splits/base_classes.txt"
    labels_emb_path = "/BS/feat_augm/work/metadata/kinetics/label_embeddings"
    ...
    test_labels_path = (
        "/BS/feat_augm/work/metadata/kinetics/data_splits/test_classes.txt"
    )
    ...
    _, features, _, _ = framework(None, features.to("cuda"), None, 5)
...
def vis_tsne_novel():
    ...
    labels_emb_path = "/BS/feat_augm/work/metadata/kinetics/label_embeddings"
    episode_split_path = (
        "/BS/feat_augm/work/metadata/kinetics/data_splits/test_episodes_5shot_5way.json"
    )
    test_labels_path = (
        "/BS/feat_augm/work/metadata/kinetics/data_splits/test_classes.txt"
    )
    ...
    for episode in proto_episode_loader.get_episodes():
        ...
        framework, optimizer, loss = create_framework(
            512, 2048, 256, [1e-3], "cos", False
        )
        state_dict = torch.load(model_path)
        state_dict = state_dict["state_dict"]
        framework.load_state_dict(state_dict)
        ...
        # pass the metadata through the model without fine tunning
        # framework.eval()
        ...
        for epoch in range(50):
            ...
            for i, data in enumerate(train_dataloader):
                ...
                # train_f = torch.cat([elm[:3] for elm in torch.split(metadata['train_features'], 5)], dim=0).to(
                #     'cuda')
                # query_features = torch.cat([elm[2:] for elm in torch.split(metadata['query_features'], 5)], dim=0).to(
                #     'cuda')
                idxs = torch.randperm(5)[:5]
                # query_features = torch.cat([elm[idxs] for elm in torch.split(metadata['query_features'], 5)], dim=0).to(
                #     'cuda')
                labs = data["labels"].to("cuda")
                # labels = torch.cat([elm[4:] for elm in torch.split(metadata['labels'], 5)], dim=0).to(opt.device)
                label_embs = torch.stack(
                    [elm[0] for elm in torch.split(data["label_embs"], 5)], dim=0
                ).to("cuda")

                prototypes, queries, _, _ = framework(
                    train_f, query_features, label_embs, 5
                )
                batch_loss, dists, _ = loss(prototypes, queries, labs)
                loss_meter.update(batch_loss.item(), queries.shape[0])
                # prec.update_probs_proto(distances=dists, gt=metadata['labels'], mode='train')
            ...
            framework.eval()
            _, feats, _, _ = framework(
                None, torch.from_numpy(features).to("cuda"), None, 5
            )
            ...
            protos, _, _, _ = framework(
                train_features.to("cuda"), None, train_lex.to("cuda"), 5
            )
            print("proto shape")
            print(protos.shape)
            ...
            do_tsne(
                feats,
                labels,
                "AFT embedded novel classes for episode # %s" % episode["key"],
                proto_labels,
                vis_protos=True,
            )
