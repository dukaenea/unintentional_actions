...
class KineticsAndFails(VisionDataset):
    ...
    def __init__(self, fails_path, kinetics_path, frames_per_clip, step_between_clips, fps, transform=None,
        ...
        self.base_features_path = '/BS/unintentional_actions/work/data/oops/vit_features/%s_normalised' % ('val' if val else 'train')
    ...
    def _compute_clip_times(self, video_idx, start_pts, end_pts, terminal_pts, transition_time):
        ...
        transition_frame = int(transition_time * float(original_fps))
        wif = transition_frame - start_frame
        wift = int(wif / (original_fps / self.fps)) if 0 <= wif <= (end_frame-start_frame) else self.fps
        ...
        # if ret == 'frames':
        return start_frame, end_frame, original_fps, start_time, end_time, wift
        # elif ret == 'times':
            # return start_time, end_time, original_fps

    def _read_video_features(self, video_path, video_idx, clip_idx,  start_pts, end_pts, terminal_pts, transition_time):
        ...
        start_frame, end_frame, original_fps, start_time, end_time, wif = self._compute_clip_times(video_idx, start_pts, end_pts, terminal_pts, transition_time)
        ...
        return video, start_time, end_time, wif
    ...
    def _get_clip(self, clip_idx, transition_time):
        ...
        video_clip, start_time, end_time, wif = self._read_video_features(video_path, video_idx, clip_idx, start_pts, end_pts, terminal_pts, transition_time)
        return video_clip, start_time, end_time, wif
    ...
    def __getitem__(self, idx):
        ...
        t_fail = sorted(self.fails_data[os.path.splitext(os.path.basename(video_path))[0]]['t'])
        t_fail = t_fail[len(t_fail) // 2]
        video, start_time, end_time, wif = self._get_clip(idx, t_fail)
        # print(video_path)
        try:
    ...
    for fps in args.fps_list:
        ...
        if args.val:
            cache_path = os.path.join(args.dataset_path,
                                      '{3}{2}{1}{0}{4}_videoclips_clean.pth'.format('val' if args.val else 'train',
        ...
        dataset = KineticsAndFails(video_clips=clips, fns_to_remove=fns_to_remove, t_units=t_units, **vars(args))
        # if not args.val:
