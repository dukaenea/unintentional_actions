...
if __name__ == "__main__":
    with open('JSON PATH HERE') as f:
        data = json.load(f) # this is the file with the predicted data
    ...
    anns_path = 'PATH TO/all_mturk_data.json' # this is the one we use for annotations
    human_anns_path = 'PATH TO/fourth/all_mturk_data.json' # we do not use this heldout one
    fps = 16 # same as ours

    fndata = defaultdict(list) # filaname data

    confusion = torch.zeros((3, 3))  # confusion[gt][pred]

    for d in data: # basically load all the filenames ad construct the confusion matrix
        fn = os.path.basename(d['fn'])
        fn = os.path.splitext(fn)[0]
        fndata[fn].append(d)
        confusion[d['y']][d['y_hat']] += 1

    for fn in fndata: # sort the filenames by the clip start time
    ...
    catk = list(category_fns.keys())[9] # basically catk = 'unexpected'

    filter_fns = category_fns[catk] # get all the filenames that are part of the 'unexpected' category

    # turn urls to file names
    filter_fns = list(
    ...
    for fn, ann_data in anns.items():
        if fn not in fndata or (filter_fns is not None and fn not in filter_fns): # if fn not in the saved preds fn or (we are filtering filenames and finename is not there)
            continue
        if filter_fns is not None: # if we are evaluating on a single category
            filter_fns_reached += 1 # increment result eval counter
        fail_t_gt = statistics.median(ann_data['t']) # gt time is the median of the gt times list
        if not 0.01 <= statistics.median(ann_data['rel_t']) <= 0.99: # continue if we are too close at the end of the video or too close at the begining
            continue
        if fail_t_gt < 0: # if there is no unint action, continue
            continue
        data = fndata[fn] # get the prediction data using the filename
        ...
        # continue
        max_el = max(data, key=lambda el: el['y_hat_vec'][1]) # get the prediction that has the max confidence
        # fail_t_pred = human_anns[fn]['t'][0]
        fail_t_pred = (max_el['t_start'] + max_el['t_end']) / 2 # predicted time is the mean between start and end of the clip with the most confidence
        ...
        acc_cls = 100 * (min(abs(fail_t_pred - t) for t in ann_data['t']) <= .25) # if the min of the diff between pred fail_t and all ann_t is les than some time then classity as correct
