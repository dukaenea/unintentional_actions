...
class CRF(nn.Module):
    ...
    def _compute_log_alpha(self,
        ...
        if run_backwards:
            ...
            log_prob = [self.end_transitions.expand(emissions.shape[1], -1)]
...
if __name__ == '__main__':
    ...
    train_emissions = torch.randn(batch_size, seq_length, num_tags)
    test_emissions = torch.randn(batch_size, seq_length, num_tags)
    train_labels = train_emissions.max(dim=-1)[1]
    test_labels = test_emissions.max(dim=-1)[1]
