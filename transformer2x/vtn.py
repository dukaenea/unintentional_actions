...
class GlobalVTN(nn.Module):
    ...
    def _local_forward(self, x, position_ids, pure_nr_frames, multi_scale):
        clip_encodings = self.local_model(
            x.squeeze(), position_ids, None, pure_nr_frames, multi_scale=multi_scale
        )
        ...
        if for_crf:
            ...
            if self.training:
                ...
                if opt.crf_margin_probs:
                    ...
                    return (
                        torch.tensor(list(chain.from_iterable(mls))).to(x.device),
                        loss,
                    )
        else:
            if local:
                if opt.backbone == "vit_longformer":
                    clip_encodings, pure_nr_frames = self._local_forward(
                        x, position_ids, pure_nr_frames, multi_scale
                    )
                    return clip_encodings, pure_nr_frames
                elif opt.backbone == "r3d_18":
                ...
                outs = self.global_model(
                    x,
                    position_ids,
                    pure_nr_frames=pure_nr_frames,
                    num_clips=num_clips,
                    video_level_pred=video_level_pred,
...
def create_model_trn_2x(num_classes=None, pretrained=False, pretrain_scale="frame"):
    ...
    if opt.optim == "adam":
        if opt.rep_learning:
            optimizer = torch.optim.AdamW(
                model.parameters(), weight_decay=opt.weight_decay
            )
        else:
            optimizer = torch.optim.AdamW(
                params=model.parameters(), lr=opt.lr, weight_decay=opt.weight_decay
            )
    ...
    if opt.rep_learning:
        ...
        loss = nn.CrossEntropyLoss(
            weight=torch.FloatTensor(
                [19670 / 18069, 19670 / 4137, 19670 / 19670]
            ).cuda()
        )
