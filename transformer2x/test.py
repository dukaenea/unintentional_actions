...
def test(**kwargs):
    ...
    with torch.no_grad():
        ...
        for idx, data in iterator:
            ...
            if opt.multi_scale:
                ...
                out = model(videos, position_ids, None, pure_nr_frames, labels, local=False,
                            multi_scale=opt.multi_scale, return_features=False)
            else:
            ...
            if opt.use_crf:
                ...
                if opt.crf_margin_probs:
                    probs = model(videos, position_ids, None, pure_nr_frames, labels, num_clips, False, True)
                    ...
                    out, _loss = model(videos, None, None, pure_nr_frames, labels, None, False, True)
                    _loss = _loss.mean()
                    labels = labels.flatten()
                    mask = (labels != -1)
                    labels = labels[mask]
    ...
    return {'top1': (1 / meter.avg) if opt.mmargin_loss else prec.top1()}  # ,  torch.softmax(acc_t, dim=0)
    ...
    for idx, pnf in enumerate(pnfs):
        ...
        if video_name in video_names:
            if idx == 0:
                ...
                scores = outs[int(pnfs[:idx].sum().item()): int(pnfs[:idx].sum().item()) + int(pnf.item())]
                l = labels[int(pnfs[:idx].sum().item()): int(pnfs[:idx].sum().item()) + int(pnf.item())]
            ...
            plt.axis('equal')
            plt.axis('off')
            plt.savefig(os.path.join(vis_path_plot, video_name + '_crf.svg'), bbox_inches='tight', pad_inches=0, dpi=1000)
            plt.clf()
...
def keep_cleanest_video(outs, labels, best_scores, best_lengths, pnfs, best_out, batch, best_t, best_times,
    ...
    for idx, pnf in enumerate(pnfs):
        ...
        if mean_zeros > best_scores[0] and mean_ones > best_scores[1] and mean_twos > best_scores[
            2] and mean_zeros < 0.75 and mean_ones < 0.75 and mean_twos < 0.75:
            # best_scores = [mean_zeros, mean_ones, mean_twos]
            # best_lengths = lengths
            # best_out = scores
            # best_t = batch['t'][1][idx]
            # best_times = batch['times'][idx]
            clip_times = plot_scores(scores, batch['times'][idx], batch['t'][1][idx], batch['video_name'][idx])
            print(batch['video_name'][idx])
            vis_metadata.append({'video_name': batch['video_name'][idx], 'clip_times': clip_times})
            break
