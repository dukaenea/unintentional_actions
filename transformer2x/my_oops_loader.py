...
from utils.sampler import DistributedSampler, UniformClipSampler, RandomClipSampler, ConcatSampler, RandomVideoSampler
...
class KineticsAndFails(VisionDataset):
    ...
    def __init__(self, fails_path, kinetics_path, frames_per_clip, step_between_clips, fps, transform=None,
                 ...
                 load_videos=False, load_frames=False, **kwargs):
        ...
        with open(
                "/BS/unintentional_actions/nobackup/oops/oops_dataset/annotations/heldout_transition_times.json") as f:
        ...
        if video_clips is None and fails_only and labeled_fails:
            ...
            for video_idx, vid_clips in tqdm(enumerate(self.video_clips.clips), total=len(self.video_clips.clips)):
                ...
                if val:
                    ...
                    if t_fail < 0 or not 0.01 <= statistics.median(
                            self.fails_data[os.path.splitext(os.path.basename(video_path))[0]]['rel_t']) <= 0.99 or \
                            self.fails_data[os.path.splitext(os.path.basename(video_path))[0]]['len'] < 3.0:
                        self.video_clips.clips[video_idx] = torch.Tensor()
                        # self.video_clips.clips.pop(video_idx)
                        self.video_clips.resampling_idxs[video_idx] = torch.Tensor()
                        # self.video_clips.resampling_idxs.pop(video_idx)
        ...
        if data_proportion < 1:
            ...
            subset_idxs = rng.sample(range(len(self.video_clips.video_paths)),
    ...
    def padd_video_clips_collate_fn(self, batch):
        def _zeropad(tensor, pure_nr_frames, labels,  size):
            ...
            if len(tensor.shape) == 5:
                z = torch.zeros((n, tensor.shape[1], tensor.shape[2], tensor.shape[3], tensor.shape[4]))
            else:
                z = torch.zeros((n, tensor.shape[1], tensor.shape[2]))
            tensor = torch.cat((tensor, z), dim=0)
            pure_nr_frames = torch.cat((torch.LongTensor([pure_nr_frames]), torch.zeros((size-1, ))), dim=0)
        ...
        for sample in batch:
            if sample is not None:
                if sample['features'].shape[0] < max_len:
                    f, p, l = _zeropad(sample['features'], sample['pure_nr_frames'], sample['label'], max_len)
                    sample['features'] = f
                    sample['pure_nr_frames'] = p
                    sample['label'] = l
                else:
                    sample['pure_nr_frames'] = torch.cat((torch.LongTensor([sample['pure_nr_frames']]), torch.zeros((max_len-1, ))), dim=0)
            else:
                batch.pop(sample)
...
def get_video_loader_trn_2x(args):
    ...
    for fps in args.fps_list:
        ...
        cache_path = os.path.join(args.dataset_path,
                                  '{3}{2}{1}{0}{4}{5}_videoclips.pth'.format('val' if args.val else 'train',
        ...
        if args.val:
            ...
            if args.load_videos:
                ...
                sampler = RandomClipSampler(dataset.video_clips,
                                            1000000 if args.sample_all_clips else args.clips_per_video)
